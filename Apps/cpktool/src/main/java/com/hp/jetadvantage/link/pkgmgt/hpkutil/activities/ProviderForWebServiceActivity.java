...
import com.hp.jetadvantage.link.pkgmgt.hpkutil.model.WebServiceInfo;
...
public class ProviderForWebServiceActivity extends CommonLocalizationController {
    @FXML
    ...
    private void init(){
        ...
        if(info == null) info = new WebServiceInfo();
    }

    public void setValues(WebServiceInfo info) {
        btnAdd.setDisable(true);
        if (info != null) {
            this.info = info;
    ...
    protected void handleAdd(ActionEvent actionEvent) {
        ...
        mainWindow.setWebServiceInfo(info);
        close(btnAdd);
    }
