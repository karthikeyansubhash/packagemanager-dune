...
import com.hp.jetadvantage.link.packagemanager.hpkutil.test.value.OXPdSampleTest;
import com.hp.jetadvantage.link.pkgmgt.hpkutil.HpkToolForWeb;
...
public class HpkToolTestForWeb {
    ...
    public void test01_01LoadHPKFile() throws Exception {
        File installFile = Utils.getLinkForWebHPKFile();
        Connector connector = Utils.getConnector(installFile);

        assertEquals(OXPdSampleTest.APP_UUID, connector.getUuid().toString());
        assertEquals(OXPdSampleTest.APP_NAME, connector.getName());
        assertEquals(OXPdSampleTest.VENDOR_NAME, connector.getVendorName());
        assertEquals(OXPdSampleTest.APP_DATE, connector.getDate());
        assertEquals(PlatformType.LinkForWeb, connector.getPlatformType());
        assertEquals(OXPdSampleTest.INSTALL_FILE, connector.getInstallFile());
        assertEquals(OXPdSampleTest.SCHEMA_LOCATION, connector.getSchemaLocation());
    }
    ...
    /*
        ...
        public void test02_01GetConfigurationNormalCase() throws Exception {
            jsonData  = "{\"password\":\"ServicePass\",\"url\":\"https://my-cloud-print.com\",\"userid\":\"ServiceUser\"}";
            testGetConfiguration(Utils.HOST, OXPdSampleTest.APP_UUID, Utils.PASSWORD, configTaskInterface);
        }

        @Test
        public void test02_02GetConfigurationWrongPasswordCase() throws Exception {
            testGetConfiguration(Utils.HOST, OXPdSampleTest.APP_UUID, Utils.PASSWORD + Utils.WRONG_WORD, wrongConfigTaskInterface);
        }

        @Test
        public void test02_03GetConfigurationWrongUUIDCase() throws Exception {
            testGetConfiguration(Utils.HOST, OXPdSampleTest.APP_UUID + "11", Utils.PASSWORD, wrongConfigTaskInterface);
        }

        @Test
        public void test03_01UpdateConfigurationNormalCase() throws Exception {
            jsonData = "{\"password\":\"ServicePass\",\"url\":\"https://my-cloud-print.net\",\"userid\":\"ServiceUser\"}";
            testUpdateConfiguration(Utils.HOST, OXPdSampleTest.APP_UUID, Utils.PASSWORD, jsonData, configTaskInterface);
        }

        @Test
        public void test03_02UpdateConfigurationWrongPasswordCase() throws Exception {
            jsonData = "{\"password\":\"ServicePass\",\"url\":\"https://my-cloud-print.net\",\"userid\":\"ServiceUser\"}";
            testUpdateConfiguration(Utils.HOST, OXPdSampleTest.APP_UUID, Utils.PASSWORD + Utils.WRONG_WORD, jsonData, wrongConfigTaskInterface);
        }

        @Test
        public void test03_03UpdateConfigurationWrongUUIDCase() throws Exception {
            jsonData = "{\"password\":\"ServicePass\",\"url\":\"https://my-cloud-print.net\",\"userid\":\"ServiceUser\"}";
            testUpdateConfiguration(Utils.HOST, OXPdSampleTest.APP_UUID + "11", Utils.PASSWORD, jsonData, wrongConfigTaskInterface);
        }

        @Test
        public void test03_04UpdateConfigurationNullJsonCase() throws Exception {
            jsonData = "";
            testUpdateConfiguration(Utils.HOST, OXPdSampleTest.APP_UUID, Utils.PASSWORD, jsonData, wrongConfigTaskInterface);
        }
    */
    ...
    /*
     * create-button option is not supported now.

    @Test
    public void test05_03CommandCreateButtonXmlCase() throws Exception {
        File appFile = Utils.getAppFile("/xml/buttonFile.xml");

        String testPath = Utils.getTestDataFolder() + "/xml/";
        String testOutputfile = testPath + "buttonFile.xml";

        String cmd = "--create-button --output " + testOutputfile +
                " --uuid 12345678-1234-1234-0000-123456789101 --name OXPd_Sample_app --description OXPd_Sample_app_description --binding Plain --uri https://www.test.com";
        String[] args = cmd.split(" ");
        HpkToolForWeb.main(args);

        ButtonInfo buttonInfo = Utils.getButtonInfo(appFile);

        assertEquals("12345678-1234-1234-0000-123456789101", buttonInfo.getUuid().toString());
        assertEquals("OXPd_Sample_app", buttonInfo.getName());
        assertEquals("OXPd_Sample_app_description", buttonInfo.getDescription().get(0).getValue());
        assertEquals("Plain", buttonInfo.getBrowserTarget().getWebApplication().getBinding());
        assertEquals("https://www.test.com", buttonInfo.getBrowserTarget().getWebApplication().getUri());
    }
    */

    @Test
    public void test05_01CommandCreateHPKUsingXMLCase() throws Exception {
        File apkFile = Utils.getXMLFile();
        ...
        assertEquals("LinkForWeb", connector.getPlatformType().toString());
    }

    @Test
    public void test05_02CommandXMLCreateCase() throws Exception {
        File apkFile = Utils.getXMLFile();
        File appFile = Utils.getAppFile("/xml/hpk_for_web.xml");
        String testHPKPath = apkFile.getParent() + "\\test.hpk";
        ...
        HpkFile hpkFile = new HpkFile(testHPKPath);
        Connector connector = hpkFile.getConnector();
        assertEquals("cd3a44d7-bd44-4e35-b6d9-27de323b45b8", connector.getUuid().toString());
        assertEquals("OXPd Sample 10 app", connector.getName());
        assertEquals("Sample 10 ISV", connector.getVendorName());
        assertEquals("LinkForWeb", connector.getPlatformType().toString());
        assertEquals("oxpd_sample_10.xml", connector.getInstallFile());
    }
    ...
    private void testInstaller(String host, String admin, String password, final TaskInterface taskInterface) {
        ...
        installer.install(Utils.getLinkForWebHPKFile(), new InstallListener() {
            @Override
    ...
    private void testUnInstaller(String host, String admin, String password, final TaskInterface taskInterface) {
        ...
        uninstaller.uninstall(OXPdSampleTest.APP_UUID, new InstallListener() {
            @Override
            public void status(PackageInstallerState status, String cause) {
                TaskStatus taskStatus = new TaskStatus(status, cause);
                taskInterface.updateMessage(taskStatus);
    ...
    private void testGetConfiguration(String host, String uuid, String password, TaskInterface taskInterface) throws Exception {
        sleep();
        Constants.DEFAULT_USER_PASSWORD = password;
        ConfigManager configManager = new ConfigManager(host, UUID.fromString(uuid));
        configManager.getConfiguration(taskInterface);
    }

    private void testUpdateConfiguration(String host, String uuid, String password, String jsonData, TaskInterface taskInterface) throws Exception {
        sleep();
        Constants.DEFAULT_USER_PASSWORD = password;
